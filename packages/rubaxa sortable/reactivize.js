...
Template.sortable.rendered = function () {
  var templateInstance = this;
	...
	templateInstance.options.onAdd = function sortableAdd(/**Event*/event) {
		...
		//console.log(templateInstance.collection)
		if (templateInstance.collection.findOne({}, { sort: sortSpecifier, limit: 1 }) !== undefined) {
		...
		if (event.data._id !== null && templateInstance.collection.findOne({_id: event.data._id}) === undefined) {
		...
		if (event.clone !== undefined && event.clone !== null) {
